...
async function loadUser(username, action) {
    ...
    try {
        ...
        if (result.RegionCode != null) {
            sessionStorage.RegionCode = result.RegionCode.trim();
        }
        else { sessionStorage.RegionCode = "";}
        setDropdownDefaultValue('ddlRegion', sessionStorage.RegionCode);
       
        if (result.BranchCode != null) {
            sessionStorage.BranchCode = result.BranchCode.trim();
        }
        else { sessionStorage.BranchCode = ""; }
        
        setDropdownDefaultValue('ddlBranch', sessionStorage.BranchCode);
        disableBr(action);
...
function ddlRoleOnChange() {
    ...
    var reg = sessionStorage.RegionCode;
    var br = sessionStorage.BranchCode;
...
function ddlRegionOnChange() {
    ...
    loadingScreen(false, 0);
    var region;
    if (sessionStorage.RegionCode == undefined)
    {
        region = "";
    }
    else { region = sessionStorage.RegionCode; }
...
function loadBranchPerRegion() {
    ...
    loadingScreen(false, 0);
    var region = getDropdownValue("ddlRegion");
    //var region = sessionStorage.RegionCode;
    //if (sessionStorage.RegionCode == undefined) {
    //    region = "";
    //}
    //else { region = sessionStorage.RegionCode; }
    ...
    if (userData.Action == 'edit') {
        ...
        for (const key in postData) {
            if (userData.hasOwnProperty(key)) {
                if (postData[key].trim() !== userData[key].trim()) {
                    bool = false
                    break;
                }
            }
...
function unlockUser(Username) {
    ...
    showConfirmModal("unlock", function () {
        ...
        PageMethods.UnlockUser(Username,
            function (result) {
                if (result == '') {
                    showSuccessModal('unlocked.', function () {
                        closeModal('modalUser', reload);
                    });
                }
                else {
...
function clearModalFields() {
    ...
    setDropdownValue('ddlRegion', '');
    setDropdownValue('ddlBranch', '');
    sessionStorage.RegionCode="";
    sessionStorage.BranchCode="";
